...
static void constructor (LexState *ls, expdesc *t) {
  ...
  checknext(ls, TK_LBRACE);
  do {
    lua_assert(cc.v.k == VVOID || cc.tostore > 0);
    if (ls->t.token == TK_RBRACE) break;
    closelistfield(fs, &cc);
    field(ls, &cc);
  } while (testnext(ls, ',') || testnext(ls, ';'));
  check_match(ls, TK_RBRACE, TK_LBRACE, line);
  lastlistfield(fs, &cc);
...
static void funcargs (LexState *ls, expdesc *f, int line) {
  ...
  switch (ls->t.token) {
    ...
    case TK_LBRACE: {  /* funcargs -> constructor */
      constructor(ls, &args);
      break;
    }
...
static void suffixedexp (LexState *ls, expdesc *v) {
  ...
  for (;;) {
    switch (ls->t.token) {
      ...
      case TK_LPAREN: case TK_STRING: case TK_LBRACE: {  /* funcargs */
...
static BinOpr getbinopr (int op) {
  switch (op) {
    ...
    case TK_MULTIPLY: return OPR_MUL;
    case TK_MODULO: return OPR_MOD;
    case TK_CARET: return OPR_POW;
    case TK_DIVIDE: return OPR_DIV;
